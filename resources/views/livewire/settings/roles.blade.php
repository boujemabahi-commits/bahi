<div>
    <div class="flex items-center justify-between gap-3 px-6 py-4 border-b border-ink-100">
        <div>
            <h3 class="font-bold text-ink-800">{{ __('الأدوار والصلاحيات') }}</h3>
            <p class="text-xs text-ink-400 mt-0.5">{{ __('الأدوار الأساسية ثابتة؛ يمكنك إنشاء أدوار مخصصة بصلاحيات أدق لمركزك.') }}</p>
        </div>
        <button type="button" class="btn-secondary shrink-0" wire:click="openCreate"><x-icon name="plus" class="w-4 h-4" /> {{ __('دور جديد') }}</button>
    </div>
            ...
            <div class="px-6 py-4" wire:key="role-{{ $r->id }}">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="font-semibold text-ink-800 text-sm">
                            {{ $r->label }}
                            @unless ($r->is_built_in) <span class="text-[11px] font-normal text-brand-600">{{ __('مخصص') }}</span> @endunless
                        </p>
                        <p class="text-xs text-ink-400">{{ $r->description }}</p>
                    </div>
                    <span class="ltr-nums text-xs font-semibold text-ink-500 shrink-0">{{ __(':count مستخدم', ['count' => $r->users_count]) }}</span>
                    @unless ($r->is_built_in)
                        <button type="button" class="btn-icon" wire:click="openEdit({{ $r->id }})" aria-label="{{ __('تعديل') }}" title="{{ __('تعديل') }}"><x-icon name="pencil" class="w-4 h-4" /></button>
                        <button type="button" class="btn-icon text-red-600" wire:click="confirmDelete({{ $r->id }})" aria-label="{{ __('حذف') }}" title="{{ __('حذف') }}"><x-icon name="trash-2" class="w-4 h-4" /></button>
                    @endunless
                </div>
                <div class="flex flex-wrap gap-1.5 mt-3 ps-[52px]">
                    @forelse ($r->permissions->sortBy(fn ($p) => array_search($p->name, array_keys($catalog))) as $p)
                        <span class="inline-flex items-center rounded-lg bg-ink-100 px-2 py-0.5 text-[11px] font-semibold text-ink-600">{{ __($catalog[$p->name] ?? $p->name) }}</span>
                    @empty
                        <span class="text-[11px] text-ink-400">{{ __('بدون صلاحيات') }}</span>
                    @endforelse
                </div>
            </div>
        @endforeach
        ...
        <div x-data x-init="$el.querySelector('input')?.focus()" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md bg-white rounded-2xl shadow-popover overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-ink-100">
                    <h2 class="text-base font-bold text-ink-900">{{ $editingId ? __('تعديل الدور') : __('دور جديد') }}</h2>
                ...
                <form wire:submit="save" class="p-5 max-h-[70vh] overflow-y-auto space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-ink-700 mb-1.5">{{ __('اسم الدور') }}</label>
                        <input type="text" wire:model="name" class="input ps-3" placeholder="{{ __('مثال: مسؤول التسويق') }}" />
                        @error('name') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <p class="block text-sm font-medium text-ink-700 mb-2">{{ __('الصلاحيات') }}</p>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-2">
                        ...
                        <button type="submit" class="btn-primary" wire:loading.attr="disabled" wire:target="save">
                            <x-icon name="check" class="w-4 h-4" /> {{ $editingId ? __('حفظ التغييرات') : __('إنشاء الدور') }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm bg-white rounded-2xl shadow-popover overflow-hidden p-5 text-center">
                ...
                <h2 class="text-base font-bold text-ink-900 mb-1.5">{{ __('حذف الدور') }}</h2>
                <p class="text-sm text-ink-500 mb-5">{{ __('هل أنت متأكد من حذف هذا الدور؟ لا يمكن حذف دور لا يزال مسنداً إلى مستخدمين.') }}</p>
