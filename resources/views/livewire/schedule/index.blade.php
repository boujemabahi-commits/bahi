...
@endphp
<div>
    <x-page-header :title="__('الجدول الأسبوعي')" :subtitle="__('توقيت الحصص لجميع المجموعات على مدار الأسبوع')">
        <button type="button" class="btn-primary" wire:click="openCreate">
            <x-icon name="plus" class="w-4 h-4" /> {{ __('إضافة حصة') }}
        </button>
    </x-page-header>

    <div class="card p-4 mb-6 flex flex-wrap items-center gap-3">
        <select class="select sm:w-48" wire:model.live="teacherFilter">
            <option value="">{{ __('كل الأساتذة') }}</option>
            @foreach ($teachers as $t)
                <option value="{{ $t->id }}">{{ $t->name }}</option>
            @endforeach
        </select>
        <select class="select sm:w-40" wire:model.live="roomFilter">
            <option value="">{{ __('كل القاعات') }}</option>
            @foreach ($rooms as $r)
                <option value="{{ $r }}">{{ $r }}</option>
            @endforeach
        </select>
        <div class="flex items-center gap-3 text-xs text-ink-400 ms-auto">
            <span class="ltr-nums">{{ __(':count حصة', ['count' => $totalSlots]) }}</span>
            <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-brand-400"></span> {{ __('نشط') }}</span>
        </div>
    </div>
            ...
            @foreach ($days as $day)
                <div class="w-64 shrink-0">
                    <div class="flex items-center justify-between mb-3 px-1">
                        <h3 class="font-bold text-ink-800">{{ __($day) }}</h3>
                        <div class="flex items-center gap-1.5">
                            <span class="ltr-nums text-xs text-ink-400">{{ __(':count حصص', ['count' => count($week[$day] ?? [])]) }}</span>
                            <button type="button" class="btn-icon !p-1" wire:click="openCreate('{{ $day }}')" aria-label="{{ __('إضافة حصة يوم :day', ['day' => __($day)]) }}" title="{{ __('إضافة حصة') }}"><x-icon name="plus" class="w-3.5 h-3.5" /></button>
                        </div>
                    </div>
                    <div class="space-y-3">
                        @forelse ($week[$day] ?? [] as $i => $class)
                            ...
                            <div class="group relative rounded-xl border p-3.5 {{ $conflict ? 'ring-2 ring-red-400 ' : '' }}{{ $tone }}" wire:key="slot-{{ $class->id }}">
                                @if ($conflict)
                                    <span class="absolute top-2 start-2 inline-flex items-center gap-1 rounded-full bg-red-600 px-2 py-0.5 text-[10px] font-bold text-white" title="{{ $conflict === 'room' ? __('القاعة محجوزة في نفس الوقت') : ($conflict === 'teacher' ? __('الأستاذ لديه حصة أخرى في نفس الوقت') : __('تعارض في القاعة والأستاذ')) }}">
                                        <x-icon name="triangle-alert" class="w-3 h-3" /> {{ __('تعارض') }} {{ $conflict === 'room' ? __('قاعة') : ($conflict === 'teacher' ? __('أستاذ') : __('قاعة وأستاذ')) }}
                                    </span>
                                @endif
                                <div class="absolute top-2 end-2 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                                    <button type="button" class="rounded-md p-1 text-ink-500 hover:bg-white/80 hover:text-ink-800" wire:click="openEdit({{ $class->id }})" aria-label="{{ __('تعديل') }}"><x-icon name="pencil" class="w-3.5 h-3.5" /></button>
                                    <button type="button" class="rounded-md p-1 text-ink-500 hover:bg-white/80 hover:text-red-600" wire:click="confirmDelete({{ $class->id }})" aria-label="{{ __('حذف') }}"><x-icon name="trash-2" class="w-3.5 h-3.5" /></button>
                                </div>
                                <p class="ltr-nums text-xs font-bold mb-1.5">{{ $class->time }}</p>
                                <p class="text-sm font-bold text-ink-900">{{ $class->course?->name ?? '—' }}</p>
                                <p class="text-xs text-ink-500 mt-0.5">{{ $class->group?->name ?? __('بدون مجموعة') }}</p>
                                <div class="flex items-center gap-1.5 mt-2 text-xs text-ink-600">
                                    <x-icon name="graduation-cap" class="w-3.5 h-3.5" /> {{ $class->teacher?->name ?? __('بدون أستاذ') }}
                                </div>
                        ...
                        @empty
                            <div class="rounded-xl border border-dashed border-ink-200 p-4 text-center">
                                <p class="text-xs text-ink-400">{{ __('لا توجد حصص') }}</p>
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        <div x-data x-init="$el.querySelector('select')?.focus()" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md bg-white rounded-2xl shadow-popover overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-ink-100">
                    <h2 class="text-base font-bold text-ink-900">{{ $editingId ? __('تعديل الحصة') : __('إضافة حصة جديدة') }}</h2>
                ...
                <form wire:submit="save" class="p-5 max-h-[70vh] overflow-y-auto space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm font-medium text-ink-700 mb-1.5">{{ __('اليوم') }}</label>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="room" class="input ps-3" placeholder="{{ __('مثال: القاعة 1') }}" list="schedule-rooms" />
                    ...
                    <div class="flex items-center justify-end gap-2 pt-2">
                        ...
                        <button type="submit" class="btn-primary" wire:loading.attr="disabled" wire:target="save">
                            ...
                            {{ $editingId ? __('حفظ التعديلات') : __('إضافة الحصة') }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm bg-white rounded-2xl shadow-popover overflow-hidden p-5 text-center">
                ...
                <h2 class="text-base font-bold text-ink-900 mb-1.5">{{ __('حذف الحصة') }}</h2>
                <p class="text-sm text-ink-500 mb-5">{{ __('هل أنت متأكد من حذف هذه الحصة من الجدول؟') }}</p>
